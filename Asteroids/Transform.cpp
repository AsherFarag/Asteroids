...
void Transformation::UpdateTransform(bool a_Force) const
{
	if (m_Dirty || a_Force)
	{
		...
		m_LocalMatrix = MatrixMultiply(MatrixMultiply(Translation, Rotation), Scale);
...
{
	UpdateTransform(false);
	return Vec3{ m_GlobalMatrix.m8, m_GlobalMatrix.m9, 0.0f };
}
...
float Transformation::GetGlobalRotation() const
{
	...
	TempGlobal.m8 = 0; TempGlobal.m9 = 0; // Sets the Translation to 0 for Both X and Y

	Vec3 MagnitudeVector = Vec3{ TempGlobal.m0, TempGlobal.m1, 0.0f };
	float XMagnitude = Vector3Length(MagnitudeVector);		        // Calculates the Scale of the First Row in Temp Global 
	TempGlobal.m0 /= XMagnitude; TempGlobal.m1 /= XMagnitude; // Seperates the Rotation of m00 and m01 from the TempGlobal by Dividing with the Scale

	return (float)std::atan2(TempGlobal.m0, TempGlobal.m1);
}
...
const Vec3& Transformation::GetGlobalScale() const
{
	...
	float XMagnitude = Vector3Length( Vec3{TempGlobal.m0, TempGlobal.m1, 0.0f } ); // Calculates the Scale of the First Row in Temp Global 
	float YMagnitude = Vector3Length(Vec3{ TempGlobal.m4, TempGlobal.m5 }); // Calculates the Scale of the Second Row in Temp Global 
